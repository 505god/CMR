...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        _detailLab.textColor = [UIColor colorWithRed:0.6549 green:0.8078 blue:0.2902 alpha:1];
        _detailLab.backgroundColor = [UIColor clearColor];
        _detailLab.font = [UIFont systemFontOfSize:10];
        
        _titleLab.text = @"自动提醒已设置";
...
-(void)layoutSubviews {
    ...
    switch (_msgStyle) {
        ...
        {
            [_userHead setFrame:CGRectMake(CELL_WIDTH-INSETS-HEAD_SIZE, INSETS*2+TIME_HEIGHT,HEAD_SIZE , HEAD_SIZE)];
            ...
            if (_messageType==CMRMessageTypeRemind) {
                NSString *content = [self getContentfronMessage:orgin];
                CGSize textSize=[content sizeWithFont:[UIFont systemFontOfSize:12] constrainedToSize:CGSizeMake((320-HEAD_SIZE*2-3*INSETS-60), TEXT_MAX_HEIGHT) lineBreakMode:NSLineBreakByWordWrapping];
                ...
                _titleLab.hidden = NO;
                [_messageConent setFont:[UIFont systemFontOfSize:12]];
                [_messageConent setTextColor:[UIColor grayColor]];
                _bubbleBg.frame=CGRectMake(CELL_WIDTH-INSETS*2-HEAD_SIZE-width-60, INSETS*2+TIME_HEIGHT, width+60, textSize.height+INSETS*4+40);
                
                [_titleLab setFrame:CGRectMake(_bubbleBg.frame.origin.x+45, _bubbleBg.frame.origin.y+INSETS, 110, 18)];
                [_messageConent setFrame:CGRectMake(_bubbleBg.frame.origin.x+45, _titleLab.frame.origin.y+18, width, textSize.height)];
                [_detailLab setFrame:CGRectMake(_bubbleBg.frame.origin.x+45, _messageConent.frame.origin.y+_messageConent.frame.size.height+INSETS, 120, TIME_HEIGHT)];
                ...
                NSString *content = [self getContentfronMessage:orgin];
                CGSize textSize=[content sizeWithFont:[UIFont systemFontOfSize:15] constrainedToSize:CGSizeMake((320-HEAD_SIZE*2-3*INSETS-60), TEXT_MAX_HEIGHT) lineBreakMode:NSLineBreakByWordWrapping];
                CGFloat height = 0;
                ...
                _bubbleBg.frame=CGRectMake(CELL_WIDTH-INSETS*2-HEAD_SIZE-textSize.width-60, INSETS*2+TIME_HEIGHT, textSize.width+60, height+INSETS*6);
                ...
                CGSize textSize=[orgin sizeWithFont:[UIFont systemFontOfSize:15] constrainedToSize:CGSizeMake((320-HEAD_SIZE*2-3*INSETS-60), TEXT_MAX_HEIGHT) lineBreakMode:NSLineBreakByWordWrapping];
                CGFloat height = 0;
                if (textSize.height-30<0) {//最少30
                    height=30;
                }else {
                ...
                _bubbleBg.frame=CGRectMake(CELL_WIDTH-INSETS*2-HEAD_SIZE-textSize.width-60, INSETS*2+TIME_HEIGHT, textSize.width+60, height+INSETS*4);
        ...
        case CMRMessageCellStyleOther:
        {
            if (_mediaType == CMRmediaTypeText) {
                CGSize textSize=[orgin sizeWithFont:[UIFont systemFontOfSize:15] constrainedToSize:CGSizeMake((320-HEAD_SIZE-3*INSETS-60), TEXT_MAX_HEIGHT) lineBreakMode:NSLineBreakByWordWrapping];
                CGFloat height = 0;
                if (textSize.height-30<0) {//最少30
                    height=30;
                }else {
                ...
                [_messageConent setFont:[UIFont systemFontOfSize:15]];
                [_messageConent setTextColor:[UIColor blackColor]];
                [_userHead setFrame:CGRectMake(INSETS, INSETS*2+TIME_HEIGHT,HEAD_SIZE , HEAD_SIZE)];
                
                [_bubbleBg setImage:[[UIImage imageNamed:@"ReceiverTextNodeBkg"]stretchableImageWithLeftCapWidth:20 topCapHeight:30]];
                _bubbleBg.frame=CGRectMake(2*INSETS+HEAD_SIZE, INSETS*2+TIME_HEIGHT, textSize.width+30, height+INSETS*4);
                ...
                [_chatImage setHidden:NO];
                [_bubbleBg setImage:[[UIImage imageNamed:@"ReceiverTextNodeBkg"]stretchableImageWithLeftCapWidth:20 topCapHeight:30]];
                _bubbleBg.frame=CGRectMake(2*INSETS+HEAD_SIZE, INSETS*2+TIME_HEIGHT, 107, 120);
                ...
                [_bubbleBg setImage:[[UIImage imageNamed:@"ReceiverTextNodeBkg"]stretchableImageWithLeftCapWidth:20 topCapHeight:30]];
                _bubbleBg.frame=CGRectMake(2*INSETS+HEAD_SIZE, INSETS*2+TIME_HEIGHT, 57, 50);
...
-(void)setMessageObject:(CMRMessageObject *)aMessage
{
    ...
    if (_messageType == CMRMessageTypeRemind) {
        [_detailLab setText:[NSString stringWithFormat:@"于%@发送",aMessage.messageDate]];
    }
}
-(void)setHeadImage:(NSString *)head
{
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"http://116.255.202.113:3002%@",head]];
